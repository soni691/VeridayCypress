...
class UserManager{
    ...
    CreateTeamUser()
    {
        const button = cy.get(':nth-child(2) > a > h3')
        button.click()
        const button1 = cy.get('.btn-edit-user-view') //click on create single user button
        button1.click()
        cy.get('#teamMemberRadio').click() //click on team use radio button
        const uemail=cy.get('#emailField')
        var currentDate = new Date();
        var currentTime = currentDate.getTime();   
        var commentText = 'Automation' + currentTime + '@email.com';
       ...
       cy.get('#branchSelector').click //click on branch selector dropdown
       cy.get('select').select('  Bankers Hall, West Tower, Suite 4430')
       cy.get('#da-phones-container > .da-phones-list > :nth-child(1) > .controls > .bfh-phone').type(8530466046)
       cy.get('#teamNameField').type('AutomationTeam')
       cy.contains('Location').scrollIntoView()
       cy.get('.button').click() // click on Save & activate button
        return this
    }
